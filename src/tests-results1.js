var tests = [
    {
        source_file: './data/mbostock_bars.json',
        target_file: './data/my_line.json'
    },
    {
        source_file: './data/my_line.json',
        target_file: './data/kuijjer_line.json'
    },
    //{
    //    source_file: './data/mbostock_bars.json',
    //    target_file: './data/result_scatter_1.json'
    //},
    //{
    //    source_file: './data/result_scatter_1.json',
    //    target_file: './data/mbostock_bars.json'
    //},
    //{
    //    source_file: './data/result_scatter_1.json',
    //    target_file: './data/vallandingham_scatter.json'
    //},
    //{
    //    source_file: './data/vallandingham_scatter.json',
    //    target_file: './data/result_scatter_1.json'
    //},
    //{
    //    source_file: './data/result_scatter_1.json',
    //    target_file: './data/result_scatter_2.json'
    //},
    //{
    //    source_file: './data/result_scatter_2.json',
    //    target_file: './data/result_scatter_1.json'
    //},
    //{
    //    source_file: './data/5308_bars.json',
    //    target_file: './data/mbostock_bars.json'
    //},
    //{
    //    source_file: './data/mbostock_bars.json',
    //    target_file: './data/5308_bars.json'
    //},
    //{
    //    source_file: './data/npr_bars.json',
    //    target_file: './data/5308_bars.json'
    //},
    //{
    //    source_file: './data/5308_bars.json',
    //    target_file: './data/npr_bars.json'
    //},
    //{
    //    source_file: './data/excel_bars.json',
    //    target_file: './data/gdp_bars.json'
    //},
    //{
    //    source_file: './data/gdp_bars.json',
    //    target_file: './data/excel_bars.json'
    //},
    //{
    //    source_file: './data/dowson_bars.json',
    //    target_file: './data/leondutoit_dots.json'
    //},
    //{
    //    target_file: './data/dowson_bars.json',
    //    source_file: './data/leondutoit_dots.json'
    //},
    //{
    //    source_file: './data/example_bars.json',
    //    target_file: './data/npr_bars.json'
    //},
    //{
    //    source_file: './data/npr_bars.json',
    //    target_file: './data/example_bars.json'
    //},
    //{
    //    source_file: './data/excel_bars.json',
    //    target_file: './data/example_bars.json'
    //},
    //{
    //    source_file: './data/result_bars_1.json',
    //    target_file: './data/5308_bars.json'
    //},
    //{
    //    source_file: './data/5308_bars.json',
    //    target_file: './data/result_bars_1.json'
    //},
    //
    //{
    //    source_file: './data/excel_bars.json',
    //    target_file: './data/pew_bars.json'
    //},
    //{
    //    source_file: './data/pew_bars.json',
    //    target_file: './data/excel_bars.json'
    //},
    //{
    //    source_file: "./data/result_scatter_2.json",
    //    target_file: "./data/leondutoit_dots.json"
    //},
    //{
    //    source_file: "./data/leondutoit_dots.json",
    //    target_file: "./data/result_scatter_2.json"
    //},
    //{
    //    source_file: "./data/result_dots_1.json",
    //    target_file: "./data/result_dots_2.json"
    //},
    //{
    //    source_file: "./data/result_dots_2.json",
    //    target_file: "./data/result_dots_1.json"
    //},
    //{
    //    source_file: "./data/result_scatter_1.json",
    //    target_file: "./data/result_dots_1.json"
    //},
    //{
    //    source_file: "./data/result_dots_1.json",
    //    target_file: "./data/result_scatter_1.json"
    //}
];

module.exports = tests;